...
function increaseScrollTrack() {
  emit(
    'update:scrollTrack',
    Number(props.scrollTrack) + 10 > 100 ? 100 : Number(props.scrollTrack) + 10
  );
}
...
function decreaseScrollTrack() {
  emit(
    'update:scrollTrack',
    Number(props.scrollTrack) - 10 < 0 ? 0 : Number(props.scrollTrack) - 10
  );
}
</script>
...
<template>
  <div class="grid grid-cols-[auto_1fr_auto] gap-2">
    <Button
      ...
      @click="decreaseScrollTrack"
    />
    <TableRangeBar
      :model-value="props.scrollTrack"
