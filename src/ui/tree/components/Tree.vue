	...
	<Tree :pt="treeStyle" :value="value" v-model:selectionKeys="modelValue">
		<template #default="{ node }">
			<div>
				...
				<ContextMenu :pt="contextMenuStyle" ref="menu" :model="menuContextItems">
					<template #item="{ item }">
						<div
							...
							<div class="flex items-center gap-2">
								<Icon
									:name="item.icon"
									class="w-4 h-4"
									:class="[{ 'text-text-secondary': !item.iconColor }, item.iconColor]" />
								<span class="text-text-primary text-sm whitespace-nowrap">{{ item.label }}</span>
							</div>
							<span class="text-text-tertiary text-[12px] leading-[18px] whitespace-nowrap">{{
								item.hotkey
							}}</span>
						</div>
					</template>
		...
		<template #checkboxicon>
			<Icon name="checkbox" class="w-3 h-3 text-base-white" @contextmenu="onContextMenuPopup" />
		</template>
		<template #togglericon>
			<Icon name="chevron-right" class="w-5 h-5 text-fill-light" @contextmenu="onContextMenuPopup" />
		</template>
	</Tree>
</template>
