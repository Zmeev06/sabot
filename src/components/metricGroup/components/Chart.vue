<script setup lang="ts">
	...
	const getChardData = async () => {
		if (coreStore.core?.id) {
			const { data, status } = await getChart(
				coreStore.core?.id,
				'2023-06-01',
				'2023-01-01',
				regionStore.region?.id,
				searchTypeStore.searchType?.id
			);
			if (status === 200) {
				console.log(data);
			}
	...
	watch(
		() => [coreStore.core?.id, regionStore.region?.id, searchTypeStore.searchType?.id],
		() => {
