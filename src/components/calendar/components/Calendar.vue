...
const props = withDefaults(
	defineProps<{
		modelValue?: {
			start: string | number | Date | DatePickerModel;
			end:  string | number | Date | DatePickerModel;
		}
...
const startDate = ref('2023-02-01')
const endDate = ref('2023-02-05')
...
watch(date, () => {
	...
	startDate.value = date.value.start
	endDate.value = date.value.end
})
	...
	<div class="flex bg-base-white mt-1 rounded-lg border-[1px] border-grey-mid calendar-shadow">
		...
		<div class="relative">
			<div v-if="$attrs.mode !== 'time'" class="absolute top-3 z-[1] flex w-full justify-between px-6">
				<Button class="h-10 w-10 p-0" variant="tertiary" icon="chevron-left" @click="handleNav('prev')"></Button>
				<Button class="h-10 w-10 p-0" variant="tertiary" icon="chevron-right" @click="handleNav('next')"></Button>
			</div>

			<DatePicker ref="datePicker" v-bind="$attrs" v-model.range.string="date" class="calendar" :columns="2" trim-weeks
				:masks="masks" :transition="'none'">
				...
				<template #footer>
					<div class="flex items-center justify-between gap-12 p-4">
						<div class="shrink-1 flex grow-0 items-center gap-3">
							<Input class="w-[136px] !font-normal" v-model:value="startDate" />
							<div class="text-base text-fill-mid">–</div>
							<Input class="w-[136px] !font-normal" v-model:value="endDate" />
						</div>
